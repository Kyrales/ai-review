...
- [x] Зафиксировать LF для shell-runtime через `.gitattributes` и выполнить полный `pytest -q` в Windows-среде; итог после последующих регрессий: `957 passed`.
...
- [x] Собрать и опубликовать защищённый Docker-образ. Merge commit `7946372`, workflow `Publish GitFlic fork` run `34327096444`: тесты, protected-runtime smoke, публикация, Cosign-подпись и анонимный pull — PASS.

### Дополнительные регрессии, выявленные на MR 56

- [x] Исключить содержимое типовых шаблонов ограничений ролей 1С в `Rights.rights`: `ПоЗначениям`, `ПоЗначениямРасширенный`, `ПоЗначениямИНаборамРасширенный`.
- [x] Не считать комментарий между продолжающими строками открытого многострочного литерала BSL синтаксической ошибкой. Детерминированный фильтр проверяет расширение `.bsl`, узкую формулировку замечания, фактически открытый литерал и следующую значимую строку с `|`.
- [x] Применить фильтр к обоим путям публикации — inline и context — и сохранить похожие реальные замечания.
- [x] Провести два раунда ревью `sol high`; замечания первого раунда исправлены, второй раунд завершён без P1/P2.
- [x] Влить исправление через `Kyrales/ai-review#11`; upstream PR `Nikita-Filonov/ai-review#126` обновлён автоматически.
- [x] Проверить повторный полный прогон MR 56 в GitFlic job 2397: 36 файлов проанализированы, 7 пропущены правилами, все 7 замечаний опубликованы с привязкой к строкам, fallback и непривязанных замечаний нет.
- [x] Не публиковать утверждение об отсутствии прав у файла роли `.mdo`, когда соседний `Rights.rights` содержит хотя бы одно разрешённое право. Проверять только узкую формулировку и не скрывать замечания о конкретных избыточных правах.
- [x] Не считать совпадение идентификаторов элемента и реквизита управляемой формы ошибкой: это разные области идентификаторов. Учитывать также вложенные элементы формы, не скрывать реальные дубли внутри одной области и не подавлять замечание при ошибке разбора XML.
- [x] Ограничить дополнительные чтения snapshot только файлами-кандидатами `.bsl`, `Form.form` и соседним `Rights.rights`; обычные файлы не создают дополнительный `git show`.
- [x] Закрыть четыре подтверждённых ложных дискуссии MR 56 с пояснениями: две о правах/ID формы и две об обязательной подсистеме `УправлениеДоступом`.
- [x] Выполнить полный набор после регрессий: `963 passed`, Ruff и `git diff --check` — PASS.
- [x] Провести ещё два последовательных раунда ревью `sol high`: в первом исправлены рекурсивный обход элементов формы и лишние чтения файлов; второй завершён без P1/P2.
- [x] По trace pipeline 611 / job 2451 подтвердить повторное падение `Pagination exceeded max_pages=5`: control checkout содержал `10`, но runtime использовал встроенный в образ `ci/sppr/.ai-review-ones.yaml`.
- [x] Установить `vcs.pagination.max_pages: 10` в фактически используемом embedded-конфиге и закрепить runtime-тестом. MR 57 требует 7 страниц для 629 изменений.
- [x] Подключить согласованный `project-rules.md` ко всем четырём embedded-промптам; прежний файл из control checkout также не попадал в runtime.
