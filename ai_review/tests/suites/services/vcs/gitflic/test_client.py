...
def gitflic_http(monkeypatch: pytest.MonkeyPatch):
    client = SimpleNamespace(
        get_mr=AsyncMock(return_value=GitFlicMergeRequest(
            id="mr", localId=41, title="Title",
            sourceBranch=GitFlicBranch(id="feature", title="feature", hash="source"),
    ...
    monkeypatch.setattr(
        ...
        SimpleNamespace(pipeline=SimpleNamespace(owner="rt-vt", project="sppr", merge_request_id=41)),
    )
...
async def test_client_maps_review_and_threads(gitflic_http):
    ...
    assert (info.id, info.base_sha, info.head_sha, info.changed_files) == (41, "base", "head", ["a.bsl"])
    assert [(thread.kind, thread.id, len(thread.comments)) for thread in inline_threads] == [
        (ThreadKind.INLINE, "inline", 2),
    ]
    assert [(thread.kind, thread.id) for thread in general_threads] == [(ThreadKind.SUMMARY, "general")]
