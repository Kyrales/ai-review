...
def test_defaults_and_disabled():
    cfg = KnowledgeConfig()
    assert cfg.enabled is False and cfg.max_rules_per_reply == 3
...
def test_reviewer_names_are_nonempty_strings(value):
    with pytest.raises(Exception):
        TrustedReviewersConfig(gitlab=[value])

def test_reviewer_allowlist_must_be_a_list():
    with pytest.raises(Exception):
        TrustedReviewersConfig(gitlab="alice")

@pytest.mark.parametrize("value", [0, 4])
def test_max_rules_bounds(value):
    with pytest.raises(Exception):
        KnowledgeConfig(max_rules_per_reply=value)
...
def test_rules_path_rejects_unsafe_values(tmp_path: Path, rules_file: str):
    with pytest.raises(ValueError):
        resolve_rules_path_from_config(tmp_path / "config.yaml", tmp_path, rules_file)
